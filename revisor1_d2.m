...
% y realizamos un análisis de sensibilidad respecto a d2.
...
d2 = 0:0.01:1; % Ahora estamos variando d2
m4 = length(d2); % Longitud de d2

% Inicializar matrices para almacenar resultados
R4 = zeros(m4, 7); % Primera columna: d2, otras 6: valores finales de Yv
umbral = 1e-6; % Umbral para considerar valores como cero

for i = 1:m4
    P = [r0 s0 K n c d1 d2(i) b theta1 theta2 theta3 theta4 p q0 e cx cy cz G1 G2 G3];
    ...
    % Guardar en R4
    R4(i, :) = [d2(i), valores_finales];
end

%% Cálculo de métricas de variación

% 1️⃣ Rango de variación
rango_variacion = max(R4(:, 2:end)) - min(R4(:, 2:end));

% 2️⃣ Variación relativa (%)
variacion_relativa = ((max(R4(:, 2:end)) - min(R4(:, 2:end))) ./ min(R4(:, 2:end))) * 100;
variacion_relativa(isinf(variacion_relativa)) = NaN; % Evitar divisiones por 0

% 3️⃣ Sensibilidad numérica (derivada aproximada respecto a d2)
sensibilidad = diff(R4(:, 2:end)) ./ diff(d2');
...
%% Graficar la variación de Y respecto a d2
figure(1);
hold on;
plot(d2, R4(:, 2), '-b', 'LineWidth', 2); % Prey
plot(d2, R4(:, 3), '-g', 'LineWidth', 2); % Native predator
plot(d2, R4(:, 4), '-r', 'LineWidth', 2); % Exotic predator
...
figure(4);
hold on;
plot(d2(1:end-1), sensibilidad(:, 1), '-b', 'LineWidth', 2); % Prey
plot(d2(1:end-1), sensibilidad(:, 2), '-g', 'LineWidth', 2); % Native predator
plot(d2(1:end-1), sensibilidad(:, 3), '-r', 'LineWidth', 2); % Exotic predator
xlabel('$d_{2}$' , 'Interpreter', 'latex');
ylabel('Sensitivity', 'Interpreter', 'latex');
%title('Sensibilidad numérica (dY/dd2)');
legend('Prey', 'Native predator', 'Exotic predator', 'Interpreter', 'latex');
set(gca, 'FontSize', 20);
hold off;
