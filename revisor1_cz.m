...
% y realizamos un análisis de sensibilidad respecto a cz.
...
cz = 0:0.1:6; % Ahora estamos variando cz
G1 = 0.01;
G2 = 0.01;
G3 = 0.01;

m12 = length(cz); % Longitud de cz

% Inicializar matrices para almacenar resultados
R12 = zeros(m12, 7); % Primera columna: cz, otras 6: valores finales de Yv
umbral = 1e-6; % Umbral para considerar valores como cero

for i = 1:m12
    P = [r0 s0 K n c d1 d2 b theta1 theta2 theta3 theta4 p q0 e cx cy cz(i) G1 G2 G3];
    ...
    % Guardar en R12
    R12(i, :) = [cz(i), valores_finales];
end

%% Cálculo de métricas de variación

% 1️⃣ Rango de variación
rango_variacion = max(R12(:, 2:end)) - min(R12(:, 2:end));

% 2️⃣ Variación relativa (%)
variacion_relativa = ((max(R12(:, 2:end)) - min(R12(:, 2:end))) ./ min(R12(:, 2:end))) * 100;
variacion_relativa(isinf(variacion_relativa)) = NaN; % Evitar divisiones por 0

% 3️⃣ Sensibilidad numérica (derivada aproximada respecto a cz)
sensibilidad = diff(R12(:, 2:end)) ./ diff(cz');
...
%% Graficar la variación de Y respecto a cz
figure(1);
hold on;
plot(cz, R12(:, 2), '-b', 'LineWidth', 2); % Prey
plot(cz, R12(:, 3), '-g', 'LineWidth', 2); % Native predator
plot(cz, R12(:, 4), '-r', 'LineWidth', 2); % Exotic predator
...
figure(4);
hold on;
plot(cz(1:end-1), sensibilidad(:, 1), '-b', 'LineWidth', 2); % Prey
plot(cz(1:end-1), sensibilidad(:, 2), '-g', 'LineWidth', 2); % Native predator
plot(cz(1:end-1), sensibilidad(:, 3), '-r', 'LineWidth', 2); % Exotic predator
xlabel('$c_z$', 'Interpreter', 'latex');
ylabel('Sensitivity', 'Interpreter', 'latex');
%title('Sensibilidad numérica (dY/dCz)');
legend('Prey', 'Native predator', 'Exotic predator', 'Interpreter', 'latex');
set(gca, 'FontSize', 20);
hold off;
